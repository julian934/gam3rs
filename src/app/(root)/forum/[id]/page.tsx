...
const Forum = ({params}:{params: Props | undefined}) => {
  ...
  return (
    ...
    <div className='flex max-sm:invisible row-start-1 col-start-6 -mt-2 ml-16 max-sm:ml-20 max-sm:mt-0 w-full h-full bg-white rotate-45 max-sm:rotate-0 ' >
    ...
    <div className='flex relative  md:-top-4 md:right-2  max-sm:left-72 max-sm:ml-12 max-sm:z-[9999] max-sm:-top-8  row-start-4 col-start-6 bg-white -rotate-45 max-sm:-rotate-45 ml-2 mt-12 w-full h-full' >
     ...
     {forumData && forumData?.threads?.length >0 &&  forumData?.threads? forumData?.threads?.map((vals:any)=><div className=' flex flex-col w-full p-4  rounded-md shadow-sm  z-[9999]' key={vals.id}  >
       ...
       <div className='flex max-sm:relative max-sm:left-56 max-sm:w-[140px] z-[9999] max-sm:h-1/3 max-sm:w-full md:-mt-4 md:py-2 px-2 md:ml-2 md:mt-0 row-start-1  col-start-4 max-sm:col-start-2 col-span-3 bg-white w-3/4 max-sm:w-full h-1/2 md:relative md:-top-4 md:h-1/2  skew-x-12 z-50 space-x-2' >
       {/* Upper Right white area & black design */}
       <div className=' bg-red-500 h-full w-1/4   self-center  -skew-x-24 z-[9999] ' >
    ...
    <div className='flex relative max-sm:-left-2 md:-top-2 md:left-2  max-sm:w-[75px] max-sm:top-0   z-[9999]   max-sm:h-full row-start-4 row-span-2 col-start-1 -ml-8 max-sm:-ml-4 mt-10 max-sm:mt-4 h-full w-full rotate-45 max-sm:rotate-45 bg-white  ' >
       {/* Left Side Lower Corner */}
    </div>
