...
const NavBar = (props: Props) => {
  ...
  return (
    <div className='flex max-sm:invisible  flex-row max-sm:flex-col bg-black h-10vw md:col-start-1 md:row-start-1 md:col-span-4 w-full top-0 self-start z-50 max-sm:z-auto' >
      <div className=' inline-block  max-sm:z-0 max-sm:mt-28 max-sm:ml-12 flex flex-col -inset-x-36 inset-y-24  md:mt-12 h-20    w-2/3   absolute md:z-0' >
           <div className='w-20 flex self-end h-20   origin-top-right -rotate-45 bottom-2  ' >
       ...
       <div className='flex max-sm:px-2 flex-row max-sm:justify-around md:self-center md:items-center md:justify-center md:z-50 md:w-3/4 ' >
         ...
         <div className='pt-4 z-40 max-sm:z-50 text-white space-y-2 md:block' >
            {/* Profile Picture if signed in, blank if signed out, with drop down menu option. */}
            {session?  <> 
              <button className=' z-30 flex flex-col  ' onClick={menu}  >
            {dataState && dataState!=undefined &&  dataState?.settings?.thumbnails?.length>0 && dataState?.settings?<Image className='rounded-3xl md:h-12 md:w-12 md:self-center max-sm:ml-10 md:justify-center  md:self-center ' width={100} height={100} src={`${dataState?.settings?.thumbnails[0]}`} alt='User Profile Pic' />:<Image width={100} height={100} className='bg-slate-200 relative md:justify-center  md:self-center   rounded-3xl md:h-12 md:w-12' src={placeholder} alt='default image' />}
        ...
        <div className='flex  max-sm:mr-4 z-50 max-sm:self-end md:block transition ease-in-out rounded-sm hover:animate-pulse bg-gradient-to-r from-red-900 via-red-500 shadow-xl  to-red-900 hover:bg-gradient-to-r hover:from-red-900 hover:via-red-300 hover:to-red-900 hover:scale-110 -skew-x-12 w-28 z-50 ' >
          ...
          {settingsMenu? <div className='relative max-sm:top-20 max-sm:right-48 md:right-40 md:top-12 z-[9999]  z-50 ' >
            <Settings/>
            </div> :<></>}
