...
export const FileUpload = ({ onChange }: { onChange?: (files: File[]) => void }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (video: any) => {
      if (video) return videoUpdate(video);
      return { message: "No Data!" };
    },
    onSuccess: () => console.log("User data updated successfully"),
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    const file:any = newFiles[0];
    const objectUrl = URL.createObjectURL(file);

    // Set the blob URL in context
    ctx.urlSet(file);
  ...
  // Optional: Video upload display
  const vidUrl = ctx.blobUrl; // directly use context
console.log('Current Data: ', vidUrl)
  return (
