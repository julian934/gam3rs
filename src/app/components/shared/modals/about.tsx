...
const About_Modal = ({user}:any) => {
  ...
  return (
    <div className='flex   max-sm:py-10  grid grid-cols-4 grid-rows-4  bg-black md:w-full md:h-1/2 flex-col rounded-lg md:justify-between' >
         <div className='flex max-sm:w-full max-sm:h-full max-sm:-mt-24 max-sm:-ml-6   col-start-1 rotate-45 max-sm:-rotate-45 row-start-2 w-1/2   -ml-8 -mt-16  2xl:-mt-80  bg-white ' >
            {/* Upper left slant */}
          </div>
          <div className='flex border-2 border-black  max-sm:-ml-20 max-sm:w-full   max-sm:h-1/2 max-sm:mt-20 max-sm:-ml-2  col-start-1 row-start-2 w-1/2 h-1/2 2xl:w-3/4  bg-white -skew-x-12 rotate-90  -ml-6 2xl:-ml-12 mt-16 2xl:mt-12' >
               {/* Mid Left gap upper*/}
          </div>
       ...
       <div className='flex md:w-full md:col-start-1 md:col-span-4 z-50    max-sm:justify-start self-center col-start-2 col-span-2 row-start-1 md:row-start-2 md:w-full md:justify-center row-span-3 md:-mt-12 z-10 p-4' >
            <div className='flex flex-col w-full self-start self-center   justify-center md:items-center  ' >
             <h1 className='text-white self-center  justify-self-center text-7xl 2xl:text-9xl' >GAM<span className='text-red-500' >3RS</span></h1>
             <h3 className='text-red-500 self-center  justify-self-center text-3xl z-50' > Join the Network  </h3>
            </div>
            </div>
            <div className='flex max-sm:h-3/5 max-sm:w-4/5  md:z-50 max-sm:-mt-44 max-sm:-mr-4 2xl:mr-12 self-center pb-2 space-around -mt-16 2xl:-mt-80 w-3/5 max-sm:w-full h-4/5  col-start-3 row-start-2 col-span-3 justify-self-end bg-white skew-x-12 space-x-4 space-y-8 px-2  z-50 ' >
          {/* Right side upper design */}
          <div className='flex w-full h-1/4 lg:h-2/5  max-sm:h-full bg-black skew-x-12 self-end max-sm:self-end' >

          </div>
          <div className='flex  w-full h-1/4 lg:h-2/5 max-sm:h-full bg-red-600 skew-x-12 self-end' >

          </div>
          <div className='flex  w-full h-1/4 lg:h-2/5 max-sm:h-full  bg-black skew-x-12 self-end' >

          </div>
          <div className='flex mw-full h-1/4 lg:h-2/5 bg-white skew-x-12 self-end' >

          </div>
      </div>
      <div className='flex border-2 border-black  z-50  w-1/6  md:-ml-2  border-2 border-black h-full self-start row-start-2 col-start-5 z-50 ' >
         {/* Rightside mid Design*/}
      </div>
      <div className='border-2 border-black  max-sm:flex max-sm:mt-2  max-sm:w-20 max-sm:-mb-24 col-start-1 row-start-4 md:self-center md:flex md:space-between w-24 h-24 -ml-12  origin-center rotate-45 md:-mb-20  bg-white' >
      ...
      <div className='flex  z-50 row-start-1 col-start-4 w-3/4 h-3/4  bg-white rotate-45 ml-32 2xl:ml-32 mt-28 2xl:-mt-2
