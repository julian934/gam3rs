...
if(dataState){
  ...
  return (
    ...
    <div className='flex md:w-full  md:h-1/2  md:mt-6 md:ml-6   max-sm:-z-40  row-start-4 col-start-6 bg-white -rotate-45 max-sm:rotate-45 ml-2 mt-12 w-full h-full' >
        {/* right side lower */}
    </div>
      <div className='flex  drop-shadow-xl flex-col rounded-md self-center  justify-self-center max-sm:justify-self-end col-start-2 col-span-4 max-sm:col-start-1 max-sm:col-span-6 max-sm:px-2 max-sm:pl-4  max-sm:row-start-1 max-sm:row-span-4 row-start-2 row-span-3 max-sm:w-full' >
        {/*  Space for Data */} 
       
        {/* <Image className='flex w-full' src={vals.thumbnail} alt={vals.title} width={100} height={100} /> */}
        <Thumbnail playbackId={`${vals.playbackID}`} />
        <h1 className='flex max-sm:self-center ' >{vals.fileName}</h1>
