...
export async function POST(request:NextRequest){
    {/* Make sure video is sent to MUX AND all video data is sent to Notifications in Gam3r Network*/}
     const client= new MongoClient(`${process.env.NEXT_PUBLIC_MONGO_DB}`);
        ...
        try {
            ...
            const result = await db.collection('gam3rs').updateOne(
                ...
                {
                    $push: { "gam3rsinfo.news": body },//Send an object with username and user URL.
                }
            );
